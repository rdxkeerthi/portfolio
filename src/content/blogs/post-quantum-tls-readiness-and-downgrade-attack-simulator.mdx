...
<Mermaid
  title="Sequence Flow: Active MITM PQ-KeyShare Stripping Attack"
  chart={`
sequenceDiagram
    autonumber
    actor Client as PQ-TLS Client<br/>(Kyber768 + X25519)
    participant MITM as Active MITM Proxy<br/>(Adversarial Engine)
    participant Server as PQ-TLS Server<br/>(Hybrid Policy Gateway)
    participant AI as AI Anomaly Engine<br/>& Blockchain Ledger

    Client->>MITM: ClientHello (Includes Kyber768 + X25519 KeyShares)
    Note over MITM: Deep Packet Inspection:<br/>Strip Kyber768 Extension (0xfe30)
    ...
    Note over AI: Entropy Delta Drop Flagged (7.91 -> 5.82)<br/>Forensic Block Written to Ledger
`}
/>
