...
graph LR
    subgraph Endpoints [Endpoint Tier]
        A1[Linux eBPF Agent]
        A2[Windows EDR Agent]
    end

    subgraph Ingestion [Ingestion Gateway]
        B[High-Throughput gRPC Gateway]
    end

    subgraph Detection [Detection & Storage]
        C1[Sigma Stream Engine]
        C2[OpenSearch Cluster]
    end

    subgraph Response [SOC Operations]
        D1[React GraphQL Workbench]
        D2[SOAR Playbook Engine]
    end
    ...
    B --> C1
    B --> C2
    C1 -->|Critical Alert| D1
    C1 -->|Trigger Playbook| D2
    D2 -.->|Isolate Host| A1
    D2 -.->|Isolate Host| A2
```
