...
graph TD
    subgraph Ingestion [Multimodal Data Ingestion]
        A1[DICOM Thoracic CT Scans]
        A2[Genomic Mutation Panel]
        A3[Clinical Demographics]
    end

    subgraph DeepLearning [Neural Feature Extractors]
        B1[3D Volumetric CNN Trunk]
        B2[2D PACS Slice Segmenter]
        B3[Genomic Projection MLP]
    end

    subgraph Fusion [Cross-Attention Fusion]
        C[Cross-Modal Tensor Fusion<br/>Grad-CAM XAI Weights]
    end

    subgraph Decision [Clinical Output]
        D1[Malignancy Risk: 0-100%]
        D2[Fleischner Clinical Action Plan]
    end
